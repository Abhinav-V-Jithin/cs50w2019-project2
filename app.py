...
def connecting():
    print("a user has been connected")
...
@socketio.on("validate user", namespace='/chat')
def validate(datapack):
    ...
    for user in registered_users:
        if user["username"] == username:
            if user["password"] == password:
                ...
                registered_users[index] = {"username":username, "password":password, "sid":request.sid, "namespace":request.namespace, "privates":user["privates"]}
    ...
    emit('error', {'message':'Invalid username or password!', 'status':'fail'}, room=request.sid, broadcast=False, namespace='/chat')
    ...
    if registered_users != []:
        for user in registered_users:
            if user["username"] == username:
                ...
                emit('error', {'message':'This username is already taken!', 'status':'fail'}, broadcast=False, room=request.sid, namespace='/chat')
                return
    registered_users.append({"username":username, "password":password, "sid":request.sid, "privates":[]})
    print("successful registration")
    emit('success', {'message':'You have successfully registered. Congratulations!', 'status':'success'}, broadcast=False, room=request.sid, namespace='/chat') # here there is a major change tp be done!
# This one tells the client there is an error. The rest of the magic happens in the client.
# Under maintanance

@socketio.on('get details', namespace='/chat')
def getUserDetails(datapack):
    ...
    emit('user details', user_detailer, broadcast=False, room=request.sid, namespace='/chat')

@socketio.on('get all channels', namespace='/chat')
def allChannels():
    print("get all channels fired")
    channeller = []
    for ch in channels:
        channeller.append(ch['name'])
    emit('all channels currently active', channeller, broadcast=False, room=request.sid, namespace='/chat')

@socketio.on('add a channel', namespace='/chat')
def addChannel(channel_pack):
    ...
    for ch in channels:
        channeller.append(ch['name'])
    emit('added a channel', channeller, broadcast=True, namespace='/chat')

@socketio.on('add a private chat', namespace='/chat')
def addPrivateChat(dataset):
    ...
    emit('added a private chat', existing_privates, room=request.sid, broadcast=False, namespace='/chat')
    emit('added a private chat', to_existing_privates, room=sid_private_to_add, broadcast=False, namespace='/chat')

@socketio.on('send private message', namespace='/chat')
def sendMessage(dataset):
    ...
    emit('private chat send', {'private':from_send, 'messages':messages}, room=room_to_send, broadcast=False, namespace='/chat')
    emit('private chat send', {'private':to_send, 'messages':messages}, room=room_to_send2, broadcast=False, namespace='/chat')

@socketio.on('send channel message', namespace='/chat')
def channelMessage(datapack):
    ...
    for ch in channels:
        ...
        index += 1
    print(messages)
    emit('channel message broadcast', {'messages':messages, 'channel':channelname}, broadcast=True, namespace='/chat')

@socketio.on('get all messages', namespace='/chat')
def previous(data):
    ...
    emit('message history', channeller, broadcast=False, room=request.sid, namespace='/chat')

@socketio.on('get all privates', namespace='/chat')
def getPreviousMessages(data):
    ...
    emit('private chat send', {'private':private, 'messages':messages}, room=room_to_send, broadcast=False, namespace='/chat')

@socketio.on('update my session id', namespace='/chat')
